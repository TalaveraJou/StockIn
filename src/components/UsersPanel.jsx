...
function Modal({ title, onClose, children }) {
  return (
    <div style={{ position:'fixed', inset:0, zIndex:300, background:'rgba(3,70,80,0.4)', backdropFilter:'blur(3px)', display:'flex', alignItems:'center', justifyContent:'center', padding:16 }}>
      <div style={{ background:'#fff', borderRadius:14, width:'100%', maxWidth:480, maxHeight:'92vh', overflow:'auto', padding:24, boxShadow:'0 20px 60px rgba(3,70,80,0.18)' }}>
...
function UserForm({ initial, onSave, onCancel }) {
  ...
  const submit = async () => {
    if (!f.fullName.trim() || (!isEdit && (!f.username.trim() || !f.password.trim())))
      return setErr('Rellena todos los campos obligatorios')
    ...
    <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
      {!isEdit && (
        <div>
          ...
          <input value={f.username} onChange={e => setF(p=>({...p, username:e.target.value}))} style={S.inp} placeholder="sin espacios, en minúsculas" />
        </div>
      )}
      <div>
        <label style={S.label}>Nombre completo *</label>
        <input value={f.fullName} onChange={e => setF(p=>({...p, fullName:e.target.value}))} style={S.inp} placeholder="Ej: María García" />
      </div>
      <div>
        <label style={S.label}>{isEdit ? 'Nueva contraseña (dejar vacío para no cambiar)' : 'Contraseña *'}</label>
        <input type="password" value={f.password} onChange={e => setF(p=>({...p, password:e.target.value}))} style={S.inp} placeholder="••••••••" />
      </div>
      <div>
      ...
      {err && <div style={{ color:T.red, fontSize:12, padding:'8px 10px', background:'rgba(220,53,69,0.07)', borderRadius:7 }}>{err}</div>}
      <div style={{ display:'flex', gap:10, justifyContent:'flex-end', marginTop:4 }}>
        <Btn variant="ghost" onClick={onCancel}>Cancelar</Btn>
        <Btn variant="primary" onClick={submit} disabled={saving}>{saving ? 'Guardando…' : isEdit ? 'Guardar cambios' : 'Crear usuario'}</Btn>
      </div>
    </div>
  )
}
  ...
  return (
    <div style={{ animation:'fadeUp 0.3s ease' }}>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:20, flexWrap:'wrap', gap:10 }}>
        <div>
          <h1 style={{ fontSize:22, fontWeight:800, color:T.brand }}>Gestión de Usuarios</h1>
          <p style={{ fontSize:12, color:T.muted }}>{users.length} usuario(s) registrado(s)</p>
        </div>
        {tab === 'users' && (
        ...
        <div style={{ ...S.card, padding:0, overflow:'hidden' }}>
          {users.map((u, i) => (
            <div key={u.id} style={{
              ...
              background: !u.active ? 'rgba(0,0,0,0.02)' : '#fff',
              opacity: !u.active ? 0.65 : 1,
            }}>
              <div style={{ width:40, height:40, borderRadius:10, background:u.active ? ROLE_COLORS[u.role] : T.muted, display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0, fontSize:16, fontWeight:700, color:'#fff' }}>
                {u.fullName?.[0]?.toUpperCase() || '?'}
              </div>
              <div style={{ flex:1, minWidth:0 }}>
                <div style={{ fontSize:13, fontWeight:600, color:T.text }}>{u.fullName}</div>
                <div style={{ fontSize:11, color:T.muted }}>
                  @{u.username} · {u.lastLogin ? `Último acceso: ${new Date(u.lastLogin).toLocaleString('es-ES',{day:'2-digit',month:'2-digit',hour:'2-digit',minute:'2-digit'})}` : 'Nunca ha accedido'}
                </div>
              </div>
              <Badge color={ROLE_COLORS[u.role]} label={ROLE_LABELS[u.role]} />
              {!u.active && <Badge color={T.muted} label="Inactivo" />}
              {u.id === currentUser.id && <Badge color={T.green} label="Tú" />}
        ...
        <div style={{ ...S.card, padding:0, overflow:'hidden' }}>
          {log.length === 0 ? (
            ...
            <div key={entry.id} style={{ display:'flex', alignItems:'center', gap:12, padding:'11px 18px', borderBottom: i<log.length-1 ? `1px solid ${T.border}` : 'none' }}>
              <div style={{ width:8, height:8, borderRadius:'50%', background: entry.action.includes('LOGIN') ? T.green : entry.action.includes('DELETE') ? T.red : T.accent, flexShrink:0 }} />
              <div style={{ flex:1, minWidth:0 }}>
      ...
      {modal && (
        <Modal title={modal === 'new' ? 'Nuevo usuario' : `Editar · ${modal.fullName}`} onClose={() => setModal(null)}>
          <UserForm initial={modal === 'new' ? null : modal} onSave={handleSave} onCancel={() => setModal(null)} />
        </Modal>
      )}
    </div>
  )
